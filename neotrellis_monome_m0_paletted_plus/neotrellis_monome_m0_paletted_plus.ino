  ...
  if (evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING) {
    ...
    if (isInited == false && x > 0) {  //-------select-the--palette---------
      ...
      if (PrintSelection == true) {
        Serial.print("color:");            //uncomment to display color number in Serial Monitor
        Serial.println(selected_palette);  //uncomment to display color number in Serial Monitor
      }
      isInited = true;
      for (int i = 0; i < 8; i++) {
